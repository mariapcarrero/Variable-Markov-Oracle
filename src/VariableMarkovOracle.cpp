...
#include "VariableMarkovOracle.h"
...
double VariableMarkovOracle::VectorDistance(double *first, double* last, double *first2) {
    double ret = 0.0;
    while (first != last) {
...
vector<double> VariableMarkovOracle::MakeWindow(int n)
{
...
vector<int> VariableMarkovOracle::AOGenerate(int i, int total_length, float q, int k)
{
...
void VariableMarkovOracle::GenerateAudio(int i, int total_length, float q, int k_interal, int hop_size, int buffer_size, string input_filename, string output_filename)
{
...
void VariableMarkovOracle::AnalyseAudio(string sfName, int hop_size, string feature)
{
...
void VariableMarkovOracle::AddState(int first_state, int state, int start_frame){
...
void VariableMarkovOracle::CreateState(int m) {
    StateComplete newstate;
    newstate.state_ = m;
    this->states_.push_back(newstate);
    //cout << "state: " << this->states_[m].state_ << endl;
}
void VariableMarkovOracle::AddTransition(int first_state, int last_state, vector <double> vector_real, int feature_state, int starting_frame) {
...
void VariableMarkovOracle::AddInitialTransition()
{
    this->CreateState(0);
...
int VariableMarkovOracle::LengthCommonSuffix( int pi_1, int pi_2)
{
...
void VariableMarkovOracle::AddFrame(int i, vector <double> vector_real, double threshold, int start_frame, int hop_size) {
...
void VariableMarkovOracle::FindBetter(vector <double> vector_real, int state_i_plus_one, int hop_size)
{
    int s = 0;
    int flag = 0, iter = 0;
